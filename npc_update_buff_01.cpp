...
void ft_update_buff_status(t_char *info, int current_dur, int duration, const char *buff_name)
{
    if (duration == 0 && current_dur == 0)
        ...
        ft_printf("%s just received a %s that lasts for a turn\n", info->name, buff_name);
    else if (duration == 1 && current_dur == 1)
        ft_printf("%s already had a %s that lasts for a turn\n", info->name, buff_name);
    else if (duration > 1 && current_dur == 0)
        ft_printf("%s just received a %s that lasts for %d turns\n", info->name, buff_name, duration);
    else if (duration > 1 && duration > current_dur)
        ft_printf("%s his %s got refreshed to %d turns\n", info->name, buff_name, duration);
    else if (duration > 1 && current_dur > duration)
        ft_printf("%s already had a better %s that lasts for %d turns\n", info->name, buff_name, current_dur);
    else if (duration == current_dur)
        ft_printf("%s already has a %s that is just as good lasting %d turns\n", info->name, buff_name, current_dur);
    return ;
}
