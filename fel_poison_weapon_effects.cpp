        ...
        t_equipment_effect *effect, t_attack_info *attack_info)
{
    ...
    if (result_check == RL_FAIL || result_check == RL_CRIT_FAIL)
    {
        ...
        pf_printf("The target is poisoned by fell poison, taking %i damage only on "
                  "application and suffering disadvantage on their attacks for the next 2 turns.", result);
    }
    return ;
}
