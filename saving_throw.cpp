...
int ft_saving_throw(t_char *info, const char *ability_score, int ability_mod, int save_mod)
{
	...
	{
		result = CRIT_FAIL;
		ft_printf("%s rolled a critical fail (%d) on his/her %s saving throw\n", info->name, roll, ability_score);
	}
	else if (roll >= 20 - info->crit.save)
	{
		result = CRIT_SUCCES;
		ft_printf("%s rolled a crit (%d) on his/her %s saving throw\n", info->name, roll, ability_score);
	}
	else
	{
		result = roll + (((ability_mod - 10) / 2) + save_mod + mod);
		ft_printf("%s rolled %d+%d+%d+%d on his/her %s saving throw for a total of %d\n",
