...
static void ft_npc_write_file_double_char(const char* msg, char** targets, ft_file& file, t_char* info)
{
    int fd = file.get_fd();
    int i = 0;
...
{
    if (DEBUG == 1)
        pf_printf("writing file for %s\n", info->name);
    if (info->flags.alreaddy_saved || info->flags.dont_save)
        return;
    std::optional<ft_file> file;
    if (fd == -1)
        file.emplace(info->save_file, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
    else
        file.emplace(fd);
    if (file->get_fd() == -1)
    {
