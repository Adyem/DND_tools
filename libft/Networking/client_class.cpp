    ...
    : client_fd(client_fd_), closed(false), client_addr()
{
    if (client_fd < 0)
    {
        throw std::invalid_argument("Invalid client socket file descriptor");
    }
...
ssize_t ft_client::send_data(const void* data, size_t size, int flags)
{
    if (is_closed())
    {
        std::cerr << "Attempt to send data on a closed connection.\n";
        return -1;
    }
...
ssize_t ft_client::receive_data(void* buffer, size_t size, int flags)
{
    if (is_closed())
    {
        std::cerr << "Attempt to receive data on a closed connection.\n";
        return -1;
    }
...
ft_string ft_client::get_client_address() const
{
    ...
    std::string address = ft_string(host) + ":" + ft_string(service);
    return address;
}
