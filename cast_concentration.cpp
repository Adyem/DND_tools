...
int ft_apply_concentration_buff(t_char *info, t_char *target, int fd[2], const char **input, t_buff *buff)
{
    ...
    if (!temp)
	{
		...
		free(target->debufs.hunters_mark.caster_name
			[ft_double_char_length(target->debufs.hunters_mark.caster_name) - 1]);
		target->debufs.hunters_mark.caster_name
			[ft_double_char_length(target->debufs.hunters_mark.caster_name) - 1] = NULL;
