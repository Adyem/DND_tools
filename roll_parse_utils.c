...
{
    ft_printf_fd(2, "%d-Error: Result is higher than %i or lower than %i\n",
...
static int ft_check_mul_overflow(int first_number, int second_number)
{
	...
	{
        if (first_number > INT_MAX / second_number || first_number < INT_MIN / second_number)
		{
...
static int ft_check_div_overflow(int first_number, int second_number)
{
    if (second_number == 0)
	{
        ft_printf_fd(2, "175-Error: Division by zero is undefined\n");
        return (1);
    }
