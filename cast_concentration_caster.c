...
char *build_file_path(const char *folder, const char *prefix, const char *name)
{
    ...
    len = ft_strlen(folder) + ft_strlen(prefix) + ft_strlen(name) + 2;
...
int add_name_if_unique(char ***caster_name, const char *name, int *count)
{
    ...
    while (i < *count)
	{
        if (ft_strcmp_dnd((*caster_name)[i], name) == 0)
            return (0);
		i++;
	}
    temp = ft_resize_double_char(*caster_name, name, 1);
    if (!temp)
	{
        ft_printf_fd(2, "309-Error allocating memory for caster name\n");
        return (1);
    }
    *caster_name = temp;
    (*count)++;
...
int ft_update_caster_name(char ***caster_name, const char *input_name, t_buff *buff)
{
    ...
    if (!(*caster_name))
	{
        *caster_name = (char **)ft_calloc(2, sizeof(char *));
        if (!(*caster_name))
		{
            ft_printf_fd(2, "308-Error allocating memory for caster name\n");
            return (1);
        }
        **caster_name = ft_strdup(input_name);
        if (!(**caster_name))
		{
            ft_printf_fd(2, "306-Error allocating memory for caster name\n");
            return (1);
        }
        count = 1;
    }
    while (count < buff->target_amount)
	{
        ...
        if (!name_input || ft_strcmp_dnd(name_input, "exit") == 0)
		{
            free(name_input);
            break ;
        }
