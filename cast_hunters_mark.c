...
int ft_apply_hunters_mark(t_char *info, t_char *target, int fd[2], const char **input)
{
	char	**temp;
	int		i;
...
static int	ft_cast_hunters_mark_open_file(int fd[2], t_char *info, t_char *target)
{
...
void	ft_cast_hunters_mark(t_char *info, const char **input)
{
	...
	if (ft_cast_hunters_mark_open_file(fd, info, target))
		return ;
	...
	if (ft_apply_hunters_mark(info, target, fd, input))
		return ;
