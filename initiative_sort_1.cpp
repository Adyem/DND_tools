        ...
        const char *line_content)
{
    ...
    if (name_part.get_error() != FT_ERR_SUCCESS)
    {
        pf_printf("257 Error allocating memory\n");
        return (1);
    }
    value_part = line.substr(equals_index + 1);
    if (value_part.get_error() != FT_ERR_SUCCESS)
    {
        pf_printf("257 Error allocating memory\n");
        return (1);
    }
    ...
    if (entry.name.get_error() != FT_ERR_SUCCESS)
    {
        pf_printf("257 Error allocating memory\n");
        return (1);
    }
...
static t_pc  *ft_build_players_from_entries(ft_vector<t_initiative_entry> &entries)
{
    ...
    while (index < total)
    {
        ...
        current->name = adv_strdup(entry.name.c_str());
        if (!current->name)
        {
            pf_printf("257 Error allocating memory\n");
            cma_free(current);
