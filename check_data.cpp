...
int ft_npc_check_info(t_char *info)
{
	...
	if (info->stats.health <= -1 || info->stats.health > info->dstats.health)
        error += (pf_printf("%s health value not found\n", info->name), 1);
	if (info->stats.health <= -1 || info->stats.temp_hp > 5000)
		error += (pf_printf("%s temp health value not found\n", info->name), 1);
	if (info->stats.str <= -1 || info->stats.str > 30)
        error += (pf_printf("%s strength value not found\n", info->name), 1);
	if (info->stats.dex <= -1 || info->stats.dex > 30)
        error += (pf_printf("%s dexterity value not found\n", info->name), 1);
	if (info->stats.con <= -1 || info->stats.con > 30)
        error += (pf_printf("%s constitution value not found\n", info->name), 1);
	if (info->stats.inte <= -1 || info->stats.inte > 30)
        error += (pf_printf("%s intelligence value not found\n", info->name), 1);
	if (info->stats.wis <= -1 || info->stats.wis > 30)
        error += (pf_printf("%s wisdom value not found\n", info->name), 1);
	if (info->stats.cha <= -1 || info->stats.cha > 30)
        error += (pf_printf("%s charisma value not found\n", info->name), 1);
	if (info->stats.turn <= -1 || info->stats.turn > 30)
        error += (pf_printf("%s turn value not found\n", info->name), 1);
	if (info->stats.phase <= -1 || info->stats.turn > 30)
        error += (pf_printf("%s phase value not found\n", info->name), 1);
	if (info->bufs.bless.duration < 0 || info->bufs.bless.duration > 50)
        error += (pf_printf("%s bless value outside of bounds\n", info->name), 1);
	if (info->initiative < -50)
        error += (pf_printf("%s initiative value outside of bounds\n", info->name), 1);
	if (info->bufs.lightning_strike.duration < 0)
        error += (pf_printf("%s lightning strike duration outside of bounds\n", info->name), 1);
	if (info->debufs.hunters_mark.amount < 0 || info->debufs.hunters_mark.amount
        ...
        error += (pf_printf("%s hunters mark data is not correct\n", info->name), 1);
    if (info->bufs.protective_winds.duration < 0)
        error += (pf_printf("%s protective winds duration outside of bounds\n", info->name), 1);
    if (info->bufs.chaos_armor.duration < 0)
        error += (pf_printf("%s chaos armor duration outside of bounds\n", info->name), 1);
    error += ft_check_equipment_slots(info);
    error += ft_npc_check_info_2(info);
    if (error)
        pf_printf("2-%s Error with the save file, please reinitialize it with the correct values\n", info->name);
    return (error);
}
